...
name: ask-matt
description: >
  Ask which skill or flow fits your situation. A router over the skills in
  this repo.
...
> **Owner preference (2026-07-10):** for new programs and major tasks, **`/wayfinder` is the default entry** in place of `/grill-with-docs`, per Matt Pocock's updated workflow (wayfinding proved more comprehensive). Wayfinder drives `/grilling` and `/domain-modeling` internally and falls back to `/grill-with-docs` when charting surfaces no fog. Authoritative order: `os/wiki/rules/program-workflow.md`.
