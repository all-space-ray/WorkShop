...
- 2026-07-11: Proved a token-free GitHub write path for read-only Cloud Agents. A self-test workflow ran every write the engineering skills need with the built-in GITHUB_TOKEN: label create, issue create+label, comment, add/remove label, assign a user, create child, sub-issue link, dependency blocked_by, close (11/11 passed). Also proved a dynamic JSON-driven "ops executor" round-trip (agent commits a request, workflow performs the writes and commits results back, agent reads them). Findings feed the All-Space-Setup fix. Test scaffolding removed; only `setup-triage-labels.yml` kept. Note: proxy-created issues are authored by github-actions[bot], and "claim @me" must pass an explicit user login.
